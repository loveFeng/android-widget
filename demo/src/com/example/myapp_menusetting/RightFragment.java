...
import com.example.myapp_menusetting.SlidingLayer.OnInteractListener;
...
//		mSpUtil = mApplication.getSpUtil();
		...
		if (mSpUtil == null)
			mSpUtil = new SharePreferenceUtil(getActivity(), SP_FILE_NAME);
		...
		mSlidingLayer.setShadowDrawable(R.drawable.shadow);
		mSlidingLayer.setShadowWidth(30);
...
/*		SlidingMenu slidingMenu = ((DemoSettingActivity) getActivity())
				.getSlidingMenu();
		// 监听菜单关闭，来关闭弹出的设置view
		if (slidingMenu != null)
...
//					.send();// 发送信息更新一下
	...
	public boolean destorySlidingLayer() {
		if (mSlidingLayer.isOpened()) {
			mSlidingLayer.closeLayer(true);
			return true;
		}
